...
## 2026-06-10 - Safe Access to Global Swarm State

**Learning:** The application’s `/chat` and `/agents` API endpoints were directly accessing shared mutable state on the global `Swarm` singleton by invoking `list(swarm.agents.keys())` without holding `self._lock`. In FastAPI's thread pool, when multiple requests iterate over the agents dictionary simultaneously while another request creates or removes an agent, it causes a `RuntimeError: dictionary changed size during iteration`. This tight coupling between the API routing layer and the swarm's internal state management exposed the application to thread-safety bugs under concurrent load.
**Action:** Decoupled the API routing layer from the `Swarm` internal dictionary implementation. Introduced a thread-safe getter method `get_agent_names(self) -> List[str]` in `opencore/core/swarm.py` that locks the internal state and returns a secure snapshot (`list(self.agents)`). Refactored all direct accesses to `swarm.agents.keys()` in `opencore/interface/api.py` to use this safe, synchronized accessor method.
